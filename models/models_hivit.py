...
# Model PARAMs 19.18M, FLOPs 4.60G with 224 input
def hivit_tiny(**kwargs):
    model = HiViT(
...
# Model PARAMs 40.53M, FLOPs 9.13G with 224 input
def hivit_small(**kwargs):
    model = HiViT(
        embed_dim=384, depths=[1, 1, 22], num_heads=6, stem_mlp_ratio=3., mlp_ratio=4., 
        rpe=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
...
# Model PARAMs 66.42M, FLOPs 15.85G with 224 input
def hivit_base22(**kwargs):
    model = HiViT(
        embed_dim=512, depths=[1, 1, 22], num_heads=8, stem_mlp_ratio=3., mlp_ratio=4., 
        rpe=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model


# Model PARAMs 290.71M, FLOPs 63.98G with 224 input
def hivit_large(**kwargs):
    model = HiViT(
        embed_dim=768, depths=[2, 2, 40], num_heads=12, stem_mlp_ratio=3., mlp_ratio=4., 
        rpe=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model


# Model PARAMs 266.76M, FLOPs 63.75G with 224 input
def hivit_large2(**kwargs):
    model = HiViT(
        embed_dim=768, depths=[4, 4, 36], num_heads=12, stem_mlp_ratio=3., mlp_ratio=4., 
        rpe=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model


# Model PARAMs 263.91M, FLOPs 61.70G with 224 input
def hivit_large1024(**kwargs):
    model = HiViT(
        embed_dim=1024, depths=[2, 2, 20], num_heads=16, stem_mlp_ratio=3., mlp_ratio=4., 
        rpe=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
